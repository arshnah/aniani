...
class DiscordPresence:
    ...
    def ensure_connected(self):
        ...
        try:
            ...
            # used to swallow this completely silently -- "RPC just
            # doesn't show up" was unreproducible with zero information
            # to go on. print() lands in the daemon's log file (see
            # spawn_rpc_daemon in gui.py) or the panel's own stdout.
            print(f"[discord_presence] connect failed (client_id={config.get('client_id')}): {e!r}", flush=True)
            self.connected = False
    ...
    def browsing(self, detail):
        ...
        try:
            ...
            print(f"[discord_presence] browsing() update failed: {e!r}", flush=True)
            self.connected = False  # stale connection -- force a reconnect next call
            return False
    ...
    def watching(self, show, ep_no, pos_seconds, duration_seconds, paused):
        ...
        try:
            ...
            print(f"[discord_presence] watching() update failed: {e!r}", flush=True)
            self.connected = False  # stale connection -- force a reconnect next call
            return False
