...
class MpvPlayer:
    ...
    def set_volume(self, percent):
        self._set("volume", percent)  # mpv's scale is already 0-100(+)

    def set_speed(self, rate):
        self._set("speed", rate)

    def _command(self, *args):
        if not self._conn:
            return
        try:
            self._conn.sendall(json.dumps({"command": list(args)}).encode() + b"\n")
        except OSError:
            pass

    def cycle_subtitle(self):
        self._command("cycle", "sub")

    def cycle_audio(self):
        self._command("cycle", "audio")
