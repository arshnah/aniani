...
DOWNLOAD_QUEUE_PATH = os.path.join(STATE_DIR, "download_queue.json")
...
def load_download_queue():
    """-> [{"anime_title", "ep_no", "source", "ep_ref"}, ...] -- only
    ever holds incomplete jobs (done/cancelled ones are never written
    here), so the GUI can re-queue whatever didn't finish the last time
    the app was closed instead of losing it silently."""
    return _load(DOWNLOAD_QUEUE_PATH) or []


def save_download_queue(jobs):
    _save(DOWNLOAD_QUEUE_PATH, jobs)


PREFS_PATH = os.path.join(STATE_DIR, "prefs.json")
DEFAULT_PREFS = {
    "source": "anidb", "player": "vlc", "volume": 100, "speed": 1.0,
    "anilist_client_id": "", "anilist_sync": False, "compact_mode": False,
}


def load_prefs():
    return {**DEFAULT_PREFS, **(_load(PREFS_PATH) or {})}
